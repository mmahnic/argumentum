...
public:
   ...
   void execute( const ParseResult& res ) override
   {
      ...
      auto acc = std::accumulate( numbers.begin(), numbers.end(), operation.second, operation.first );
      std::cout << acc << "\n";
   }
...
TEST( StaticModuleLibrary, shouldAddNumbers )
{
...
TEST( StaticModuleLibrary, shouldEchoNumbers )
{
   auto parser = argument_parser{};
   auto params = parser.params();
   parser.config().program( "staticlib" ).description( "Accumulator" );
